...
/**
 * 异常捕获类，UncaughtException处理类,当程序发生Uncaught异常的时候,有该类来接管程序,并记录发送错误报告.
 * @author YL
...
public class CrashHandler implements Thread.UncaughtExceptionHandler {
    ...
    private boolean handleException(Throwable ex) {
        ...
        new Thread() {
            ...
            public void run() {
                Looper.prepare();
                ShowToast.Short(BaseApplication.getContext().getString(R.string.crash_wrong));
                Looper.loop();
            }
        }.start();
        //收集设备参数信息
    ...
    private String saveCrashInfo2File(Throwable ex) {
        ...
        L.d("The Error Caused By:" + result);
        sb.append(result);
        // 保存文件
        try {
            long timestamp = System.currentTimeMillis();
            String time = formatter.format(new Date());
            String fileName = BaseApplication.getContext().getPackageName()+ "-" + time + "-" + timestamp + ".log";
            if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                String path = SystemConfig.creash_path;
